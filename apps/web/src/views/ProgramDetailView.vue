...
<template>
  <section class="page project-detail-page">
    <div class="page-heading"><div><span class="hero-kicker">{{ program.level }}项目 · {{ program.stage }}</span><h2>{{ program.name }}</h2><p>{{ program.description }}</p></div><div class="heading-actions"><RouterLink to="/programs"><el-button>返回项目</el-button></RouterLink><RouterLink :to="`/programs/${program.id}/circuit`"><el-button>电路图编辑器</el-button></RouterLink><RouterLink :to="`/programs/${program.id}/remote`"><el-button :icon="VideoPlay">虚拟遥控器</el-button></RouterLink><RouterLink :to="`/programs/${program.id}/edit`"><el-button type="primary" :icon="Edit">积木编程</el-button></RouterLink></div></div>
    ...
    <div class="project-detail-grid">
      ...
      <article class="panel project-section project-circuit-card"><div class="panel-title"><div><span class="section-index">02</span><h3>项目电路图</h3></div><RouterLink :to="`/programs/${program.id}/circuit`"><el-button type="primary">打开编辑器</el-button></RouterLink></div><div class="circuit-card-preview"><span>⌁</span><div><strong>{{ program.name }}专用电路</strong><p>已预置元件、端口、接线和安全检查，可拖动修改并导出 SVG。</p></div></div></article>
      <article class="panel project-section"><div class="panel-title"><div><span class="section-index">03</span><h3>真实运行流程</h3></div></div><ol class="project-workflow"><li v-for="(step, index) in program.workflow" :key="step"><span>{{ index + 1 }}</span><p>{{ step }}</p></li></ol></article>
      <article class="panel project-section"><div class="panel-title"><div><span class="section-index">04</span><h3>编程依赖</h3></div><RouterLink :to="`/programs/${program.id}/edit`"><el-button type="primary">打开积木</el-button></RouterLink></div><div class="capability-list"><span v-for="capability in program.capabilities" :key="capability">{{ capability }}</span></div></article>
      <article class="panel project-section"><div class="panel-title"><div><span class="section-index">05</span><h3>安全检查</h3></div></div><ul class="safety-checklist"><li v-for="item in program.safety" :key="item"><span>✓</span>{{ item }}</li></ul></article>
      <article class="panel project-section project-milestones"><div class="panel-title"><div><span class="section-index">06</span><h3>开发里程碑</h3></div></div><div class="milestone-list"><div v-for="milestone in program.milestones" :key="milestone.name"><span class="milestone-dot" :class="milestone.status"></span><div><strong>{{ milestone.name }}</strong><p>{{ milestone.deliverable }}</p></div><el-tag :type="milestoneType(milestone.status)" size="small">{{ milestone.status }}</el-tag></div></div></article>
    </div>
  </section>
</template>
